...
data_dir <- "/path/to/output/folder/of/CAVAMEN/"

csv_files <- list.files(data_dir, pattern = "caveman_loged_with_marker\\.csv$", full.names = TRUE, recursive = TRUE)
#print(csv_files)
...
seg_files <- list.files(data_dir, pattern = "\\.copynumber\\.caveman_loged_with_marker\\.seg$", full.names = TRUE, recursive = TRUE)
...
output_file_path <- paste0(output_dir, '211_cna_for_gistic_2.seg')

write.table(all_seg_data, file = output_file_path, sep = "\t", row.names = FALSE, quote = FALSE, col.names = TRUE)
