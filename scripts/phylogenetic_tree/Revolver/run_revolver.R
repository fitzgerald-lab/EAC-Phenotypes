...
# Exclude the two patients without clonal CCF -- the CNA looks not right, probably from insufficient ASCAT QC
input_BO_df <- input_BO_df %>% filter(patientID != "PL_176" & patientID != "AH_412")
...
ggsave(trajectories_drivers_heatmap_FULL, file = file.path(result_dir, "plots/trajectories_drivers_heatmap_full_cohort_filtered.jpg"), width = 8, height = 8, dpi = 300, units = "in")
