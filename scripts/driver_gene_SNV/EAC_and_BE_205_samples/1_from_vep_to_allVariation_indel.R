#===========================================================================================
#===========================================================================================      
######   Script to extract and combine indel consequences of the cohort   
######   Input: .vep files          
######   Output: combined_indel_consequences.csv, all_indel_count.csv                                                                      
#===========================================================================================
#===========================================================================================

# Author: Lianlian Wu
# Date: 2024-05-01
# Description: This script processes VEP output files to extract indel consequences 
#              and combines the results for downstream analysis.
...
vep_files_dir <- "/path/to/data/indel/"
output_dir <- "/path/to/results/samples_211/"
...
count_variance_each_gene <- function(vep_file_path) {
  ...
  cleaned_data <- vep_data %>% 
      select(Location, Gene, Consequence, symbol) %>% 
      mutate(sample_id = sample_id) %>%
      distinct(sample_id, Location, .keep_all = TRUE)
  # Calculate indel count
  indel_count <- nrow(cleaned_data)
  print(sample_id)
  print(indel_count)
  indel_count_df <- data.frame(sample_id = sample_id, indel_count = indel_count)

  return(list(cleaned_data = cleaned_data, indel_count_df = indel_count_df))
...
all_indel_count <- vep_files %>% 
  lapply(function(x) count_variance_each_gene(x)$indel_count_df) %>% 
  bind_rows()
...
write.csv(all_consequences, file = paste0(output_dir, "combined_indel_consequences.csv"))
write.csv(all_indel_count, file = paste0(output_dir, "all_indel_count.csv"))
