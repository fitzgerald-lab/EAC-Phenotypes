...
source_dir <- "./Genomic_analysis/data/0_call_BE_driver_genes/copy_number/BE_samples_205/"

# List all caveman adjusted CSV files
adjusted_files <- list.files(source_dir, pattern = "\\.copynumber_loged\\.csv$", full.names = TRUE, recursive = TRUE)
...
for (seg_path in adjusted_files) {
  ...
  if (file.exists(marker_path)) {
    ...
    output_file_path <- str_replace(seg_path, "copynumber_loged\\.csv$", "copynumber_loged_with_marker.csv")
